...
"""
A tool to generate a tasks_list.sh file for running aslprep
...
def get_parser():
    ...
    parser = ArgumentParser(
        description='OpenfMRI participants sampler, for ASLPREP\'s testing purposes',
...
def main():
    ...
    if opts.cmd_call is None:
        ...
        singularity_img = sorted(
            glob.glob(os.path.join(singularity_dir, 'poldracklab_aslprep_*')))
        if singularity_img:
