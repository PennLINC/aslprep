...
def cbfcomputation(metadata, mask, m0file, cbffile, m0scale=1):
    ...
    LOGGER.warning(f"m0data: {m0data.shape}")
    # compute cbf
    ...
    if hasattr(perfusion_factor, '__len__') and cbf_data.shape[1] > 1 :
        ...
        for i in range(cbf_data.shape[1]):
            LOGGER.warning(f"cbf1[:, {i}]: {cbf1[:, i].shape}")
            LOGGER.warning(f"permfactor[{i}]: {permfactor[i].shape}")
        ...
        for k in range(len(cbf_xx)):
            ...
            LOGGER.warning(f"pldx: {pldx.shape}")
    ...
    LOGGER.warning(f"tcbf: {tcbf.shape}")
