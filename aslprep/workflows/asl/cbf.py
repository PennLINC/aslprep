...
[@chappell_basil],
as implemented in *FSL* {fslversion}. BASIL computes CBF using a spatial regularization of the
estimated perfusion image and additionally calculates a partial-volume corrected CBF image
[@chappell_pvc].
""".format(
...
def init_cbfroiquant_wf(mem_gb, omp_nthreads, scorescrub=False, basil=False, name="cbf_roiquant"):
    ...
    workflow.__desc__ = """\
For each CBF map, the ROIs for the following atlases were extracted: the  Harvard-Oxford  and the
Schaefer 200 and 400-parcel resolution atlases.
"""
