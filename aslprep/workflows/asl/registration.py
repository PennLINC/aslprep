...
def init_asl_t1_trans_wf(
    ...
    name : :obj:`str`
        Name of workflow (default: ``asl_t1_trans_wf``)
    ...
    inputnode = pe.Node(
        niu.IdentityInterface(
            fields=[
                ...
                "t1w_aparc",
                "asl_split",
                "aslcontext",  # see if I can drop this
                # Transforms
                "hmc_xforms",
                "fieldwarp",
                "itk_bold_to_t1",
    ...
    if freesurfer:
        ...
        aseg_t1w_tfm = pe.Node(
            ApplyTransforms(interpolation="MultiLabel", transforms="identity", args="-v"),
            name="aseg_t1w_tfm",
            mem_gb=0.1,
        )
        # fmt:off
    ...
    workflow.connect([(asl_to_t1w_transform, threshold, [("out_files", "in_file")])])
