...
"""
    ...
    if spaces.get_spaces(nonstandard=False, dim=(3,)):
        ...
        # The standard-space reference image generated by init_bold_std_trans_wf does not account
        # for different volume types in ASL data, so we will ignore that output and call the
        # ASL-specific reference image workflow using other outputs from that workflow.
        asl_std_reference_wf = init_asl_reference_wf(
            pre_mask=True,
            name="initial_aslref_wf",
        )
        asl_std_reference_wf.inputs.inputnode.dummy_scans = config.workflow.dummy_vols
        asl_std_reference_wf.__desc__ = None
        # fmt:off
        workflow.connect([
            (reduce_asl_file, asl_std_reference_wf, [("aslcontext", "inputnode.aslcontext")]),
            (asl_std_trans_wf, asl_std_reference_wf, [
                ("outputnode.bold_std", "inputnode.asl_file"),
                ("outputnode.bold_mask_std", "inputnode.asl_mask"),
            ]),
            (asl_std_reference_wf, asl_derivatives_wf, [
                ("outputnode.ref_image", "inputnode.aslref_std"),
            ]),
        ])
        # fmt:on
