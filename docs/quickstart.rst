...
Running *ASLPrep* will require a freesurfer license file, which can be requested `here <https://surfer.nmr.mgh.harvard.edu/registration.htm>`_. Move this license to a folder in your ``$HOME`` directory (to find this location out, run this in the terminal: echo $HOME) called **license**.
