...
Select which options you have run *ASLPrep* with to generate custom language
...
.. raw:: html
   ...
   function fillCitation(){
      ...
      function cb(err, zenodoID) {
         getCitation(zenodoID, 'vancouver-brackets-no-et-al', function(err, citation) {
            $('#aslprep_citation').text(citation.substring(2));
         });
         getDOI(zenodoID, function(err, DOI) {

            if(DOI == null) {
               $('#aslprep_doi_url').text("10.5281/zenodo.852659");
               $('#aslprep_doi_url').attr('href', 'https://doi.org/' + "10.5281/zenodo.852659");
            } else {
               $('#aslrep_doi_url').text('https://doi.org/' + DOI);
               $('#aslprep_doi_url').attr('href', 'https://doi.org/' + DOI);
            }
         });
      }
   ...
   function toggle() {
        var controlElementsIds = ["freesurfer","slicetime","ss_template", "SDC", "dof"];
   ...
   <p style="font-style: italic;">
     ...
     ASLPREP version <span id="aslprep_version">latest</span> [1, 2, RRID:SCR_016216], a Nipype [3, 4, RRID:SCR_002502] based tool.
   ...
   <p style="font-style: italic;">
     ASL data was <span class="slicetime_text_true">slice time corrected using
...
The boilerplate text generated by ASLPREP is intended to allow for clear,
consistent description of the preprocessing steps used, in order to improve the
reproducibility of studies.

We fully intend for it to be copied verbatim, or modified as it needed and have released it under the
